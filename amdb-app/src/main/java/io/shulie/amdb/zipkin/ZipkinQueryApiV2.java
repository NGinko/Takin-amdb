...
import static io.shulie.surge.data.common.utils.CommonUtils.parseIntQuietly;
...
*/
    ...
    private Span rpcBasedToSpan(RpcBased rpcBased) {
        ...
        if (StringUtils.isNotBlank(rpcBased.getRpcId()) && (!"0".equals(rpcBased.getRpcId()) || !"9".equals(rpcBased.getRpcId())) && rpcBased.getRpcId().lastIndexOf(".") != -1) {
            builder.parentId(transformLengthTo16(rpcBased.getRpcId().substring(0, rpcBased.getRpcId().lastIndexOf(".")).replaceAll("\\.", "a")));
        }
     ...
     */
    private String transformLengthTo16(String code) {
        try {
            if (code.length() > 16) {
                byte[] md5s = MessageDigest.getInstance("md5").digest(code.getBytes());
                return new BigInteger(1, md5s).toString(16).substring(8, 24);
            }
        }catch (Exception e){
        }
        return code;
